    ...
    <div className="space-y-6 font-sans">
      <div className="flex flex-wrap items-center justify-between border-b border-sand-800 pb-3 gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {source === 'local-fallback' && (
            <span className="text-xs font-bold text-amber-700 bg-amber-50 border border-amber-200 px-3 py-1 rounded-lg font-mono flex items-center gap-1.5" title="Database unreachable — showing this browser's local cache instead">
              <WifiOff className="h-3.5 w-3.5" />
              OFFLINE CACHE
            </span>
          )}
          <span className="text-xs font-bold text-emerald-text bg-emerald-light border border-emerald-border px-3 py-1 rounded-lg font-mono">
      ...
      {loading ? (
        <div className="minimal-card p-12 text-center space-y-3">
          ...
          <p className="text-xs text-sand-500 font-mono">Loading robot library from Postgres...</p>
        </div>
      ) : library.length === 0 ? (
          ...
          {library.map((entry) => {
            ...
            return (
              <div key={entry.id} className={`minimal-card p-5 space-y-4 ${isActive ? 'border-emerald-border' : ''}`}>
                <div className="flex items-start justify-between gap-3 border-b border-sand-800 pb-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isActive && (
                        <span className="text-[10px] bg-emerald-light text-emerald-text px-1.5 py-0.5 rounded font-mono font-bold border border-emerald-border shrink-0">
                          ACTIVE
                        </span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-4 font-mono text-[11px] text-sand-400">
